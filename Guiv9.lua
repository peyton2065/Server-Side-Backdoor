...
-- ═══════════════════════════════════════════════════
--  4 · ANTI-CHEAT BYPASS — ADVANCED XENO VENV MODEL
-- ═══════════════════════════════════════════════════
...
-- Sophisticated Kick/Ban Hooking using Xeno Metatable Spoofing
AC.kickPreventCode = [[
pcall(function()
    ...
    -- Hidden hook for Namecall to spoof Kick/Ban requests
    oldNc = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        local method = getnamecallmethod()
        local args = {...}
        
        if not checkcaller() then
            if (method == "Kick" or method == "Ban" or method == "BanAsync") and self == Players.LocalPlayer then
                hookResult.kicked = hookResult.kicked + 1
                return nil
            end
            
            -- Spoofing WalkSpeed and JumpPower checks (Common in Rivals/Generic)
            if method == "GetAttribute" and args[1] and (args[1]:find("Speed") or args[1]:find("Velocity")) then
                return 16 -- Default walkspeed spoof
            end
        end
        return oldNc(self, ...)
    end))

    -- Disabling known AC UI and Client-side detection loops
    local function disableDetection(obj)
        if obj:IsA("LocalScript") or obj:IsA("ModuleScript") then
            local n = obj.Name:lower()
            if n:find("ac") or n:find("anti") or n:find("detect") or n:find("cheat") or n:find("guard") then
                obj.Disabled = true
    ...
    for _, v in ipairs(game:GetDescendants()) do pcall(disableDetection, v) end
    game.DescendantAdded:Connect(function(v) pcall(disableDetection, v) end)
...
-- Advanced Server Authority Spoofing
AC.serverAuthorityCode = [[
pcall(function()
    local result = {spoofed = 0, modules = 0}
    
    -- Spoof Indexing to hide modified attributes/values
    local oldIdx
    oldIdx = hookmetamethod(game, "__index", newcclosure(function(self, key)
        if not checkcaller() then
            ...
            if key == "Health" and self:IsA("Humanoid") and self.Health > self.MaxHealth then return self.MaxHealth end
        end
        return oldIdx(self, key)
    end))

    -- Clean attributes from character
    local function clean(char)
        if not char then return end
        local hum = char:FindFirstChildWhichIsA("Humanoid")
        if hum then
            for k, v in pairs(hum:GetAttributes()) do
                if k:lower():find("flag") or k:lower():find("cheat") or k:lower():find("kick") then
    ...
    clean(game.Players.LocalPlayer.Character)
    game.Players.LocalPlayer.CharacterAdded:Connect(clean)
...
-- Pipeline Neutralizer (Destroys the reporting chain)
AC.pipelineNeutralizerCode = [[
pcall(function()
    local result = {events = 0}
    local blacklist = {"report", "flag", "kick", "ban", "cheat", "log", "detect", "suspicious"}
    
    local function isSus(name)
        name = name:lower()
        for _, b in ipairs(blacklist) do if name:find(b) then return true end end
        return false
    end

    local function killRemote(obj)
        if (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) and isSus(obj.Name) then
            local old = obj.FireServer
            if obj:IsA("RemoteEvent") then
                obj.Name = "Xeno_Nullified_" .. math.random(100,999)
                -- We don't destroy to avoid some AC 'existence' checks, we just prevent firing.
    ...
    for _, v in ipairs(game:GetDescendants()) do pcall(killRemote, v) end
    game.DescendantAdded:Connect(killRemote)
...
-- Active background loop to suppress heartbeats
AC.loopProtectCode = [[
spawn(function()
    while task.wait(2) do
        pcall(function()
            -- Force character attributes to stay clean
            local char = game.Players.LocalPlayer.Character
            if char then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then
                    for k, _ in pairs(hum:GetAttributes()) do
                        if k:lower():find("violation") or k:lower():find("strike") then
                            hum:SetAttribute(k, nil)
                        end
                    end
                end
...
pcall(function()
    local killed = 0
    local keywords = {"anticheat","antiexploit","anti-cheat","anti-exploit","antihack","gameguard","cheatdetect","exploitdetect"}
...
pcall(function()
    -- Rivals uses attribute-based flag storage on the Humanoid
    local lp = game.Players.LocalPlayer
    local function clean()
        local c = lp.Character
        local h = c and c:FindFirstChildOfClass("Humanoid")
        if h then
            h.AttributeChanged:Connect(function(attr)
                if attr:lower():find("flag") or attr:lower():find("kick") then
                    h:SetAttribute(attr, nil)
                end
            end)
        end
    end
    clean()
    lp.CharacterAdded:Connect(clean)
end)]]},
    {name="Arsenal", placeIds={286090429}, desc="FPS damage calc", code=[[
pcall(function()
    for _, obj in ipairs(game:GetService("ReplicatedStorage"):GetDescendants()) do
        if obj:IsA("RemoteEvent") and (obj.Name:find("Exploit") or obj.Name:find("Kick")) then
            hookfunction(obj.FireServer, function() return end)
        end
    end
end)]]},
...
pcall(function()
    game:GetService("RunService").Stepped:Connect(function()
        pcall(function() game.Players.LocalPlayer.Character.Humanoid:SetAttribute("S_Stats", nil) end)
    end)
end)]]},
...
pcall(function()
    if game.Players.LocalPlayer.Character then
        game.Players.LocalPlayer.Character.DescendantAdded:Connect(function(v)
            if v.Name == "JumpCheck" or v.Name == "SpeedCheck" then v:Destroy() end
        end)
    end
end)]]},
...
pcall(function()
    ...
    setfflag("LuaWebServiceProxyEnabled", "False")
end)]]},
}
...
-- Main Window
local W = CFG.Window
local main = U.new("Frame",{Name="Main",Size=UDim2.new(0,W.Width,0,W.Height),Position=UDim2.new(0.5,0,0.5,0),AnchorPoint=Vector2.new(0.5,0.5),BackgroundColor3=T.BG,BorderSizePixel=0,ClipsDescendants=true,Parent=main or gui},{
...
do
    ...
    U.new("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text=table.concat({
        '<font color="#A6E3A1">Xeno Venv Model — Advanced Rivals/Generic Bypass</font>','',
        '<font color="#F9E2AF">L1:</font> <font color="#9399B2">Metatable Kick/Ban Hooking (Hidden)</font>',
        '<font color="#F9E2AF">L2:</font> <font color="#9399B2">Virtual Attribute & Speed Spoofing</font>',
        '<font color="#F9E2AF">L3:</font> <font color="#9399B2">Remote Reporting Silence (Null Fire)</font>',
        '<font color="#F9E2AF">L4:</font> <font color="#9399B2">Deep Profile Injection</font>',
        '<font color="#F9E2AF">L5:</font> <font color="#9399B2">Proactive AC Source Killer</font>',
        '<font color="#F9E2AF">L6:</font> <font color="#9399B2">Attribute Erasure Background Loop</font>',
    ...
    actionBtn(acBtns,"🛡️ Full Bypass",T.OK,1,function()
        acResultLabel.Text='<font color="#F9E2AF">Injecting Advanced Venv Bypass...</font>'
...
reg("bypass",{"acbypass"},";bypass",function()
    addHistory("→ Running advanced venv bypass...","#F9E2AF")
    task.spawn(function()
        local ok,result = AC:execute()
        if ok then
...
reg("acstatus",{},";acstatus",function()
    addHistory(AC.active and "✓ AC: ACTIVE (Xeno Venv)" or "✗ AC: inactive — ;bypass","#9399B2")
end)
...
do
    ...
    Notify("🛡️ AC: "..profile.name.." — Advanced Venv Mode Ready",T.OK)
    ...
    print(string.format([[
    ═══════════════════════════════════════
     ⚡ XENO v%s — Advanced Executor Build
     AC Profile: %s (Venv Spoofing Active)
