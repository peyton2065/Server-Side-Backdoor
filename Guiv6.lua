...
-- ★ FIX: Safe UID generation (no Http:GenerateGUID dependency)
function U.uid()
...
function AC:execute(bdRef)
    if not bdRef or not bdRef:isConnected() then return false, "No backdoor" end
    self.results = {}
...
-- ★ FIX: No `continue` keyword — using skip flag instead
function BD:scan(statusCallback, quickMode)
    ...
    while i<=totalRemotes do
        ...
        -- ★ FIX: replaced `continue` with if-else
        if #batch > 0 then
        ...
        actionBtn(ag,act[1],act[3],i,function()
            ...
            for _,plr in ipairs(targets) do
                ...
                if AC.active and (cmd=="speed" or cmd=="jpower" or cmd=="god") then
                    task.delay(0.3,function() AC:syncAfterChange(BD) end)
                end
...
do
    ...
    -- ★ FIX: Safe game name (no unprotected GetProductInfo)
    ...
    actionBtn(acBtns,"🛡️ Full Bypass",T.OK,1,function()
        if not BD:isConnected() then Notify("Not connected!",T.Err); return end
        acResultLabel.Text='<font color="#F9E2AF">Running 6-layer bypass...</font>'
    ...
    actionBtn(acBtns,"💀 Kill Scripts",T.Err,3,function()
        ...
        if m then Notify("Killed "..m.Value.." scripts",T.OK); pcall(function() m:Destroy() end)
    ...
    local mkb = actionBtn(mkc,"Kill",T.Err,3,function()
        ...
        BD:exec(string.format([[pcall(function() for _,svc in ipairs({game:GetService("ServerScriptService"),game:GetService("ServerStorage"),game:GetService("Workspace")}) do pcall(function() for _,obj in ipairs(svc:GetDescendants()) do if (obj:IsA("Script") or obj:IsA("ModuleScript")) and (obj.Name=="%s" or obj:GetFullName():find("%s")) then pcall(function() obj.Disabled=true;obj:Destroy() end) end end end) end end)]],mki.Text,mki.Text))
        Notify("Kill sent: "..mki.Text,T.OK)
    end)
...
reg("god",{},";god <plr>",function(a) for _,p in ipairs(U.findPlayers(a[1])) do local h=pRef(p.Name)..'.Character:FindFirstChildWhichIsA("Humanoid")'; ssRun("pcall(function() "..h..".MaxHealth=math.huge;"..h..".Health=math.huge end)"); if AC.active then task.delay(0.3,function() AC:syncAfterChange(BD) end) end; addHistory("→ god "..p.Name,"#A6E3A1") end end)
...
reg("jumppower",{"jp"},";jp <plr> <n>",function(a) local v=tonumber(a[2]) or 100; for _,p in ipairs(U.findPlayers(a[1])) do ssRun("pcall(function() local h="..pRef(p.Name)..'.Character:FindFirstChildWhichIsA("Humanoid");h.UseJumpPower=true;h.JumpPower='..v.." end)"); if AC.active then task.delay(0.3,function() AC:syncAfterChange(BD) end) end; addHistory("→ jp "..p.Name.."="..v,"#B4A0FF") end end)
...
reg("acstatus",{},";acstatus",function()
    addHistory(AC.active and "✓ AC: ACTIVE" or "✗ AC: inactive — ;bypass","#9399B2")
end)

reg("clearflags",{"sync"},";clearflags",function()
    if not BD:isConnected() then return end
    AC:syncAfterChange(BD)
    addHistory("→ Flags cleared","#A6E3A1")
end)
