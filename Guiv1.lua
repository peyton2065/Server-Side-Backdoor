...
XENO | Universal Server-Side Framework v4.3.0
Xeno Executor Compatible Build — 10-Layer Venv AC
]]
...
--[[
    LAYER ARCHITECTURE (10-Layer Venv Model):
    L1  — Namecall Hook (Kick/Ban/BanAsync intercept + attribute read spoof)
    L2  — Index Spoof (__index on Humanoid + velocity clamping)
    L3  — NewIndex Trap (__newindex flag-write prevention)
    L4  — InvokeClient Null (server integrity ping rejection)
    L5  — Heartbeat Connection Purge (AC loop disconnector)
    L6  — Pipeline Neutralizer (reporting remote silence)
    L7  — GC Table Scrub (hide executor artifacts from getgc)
    L8  — Game Profile Injection (per-game specific hooks)
    L9  — Proactive Script Killer + Source Replacement
    L10 — Background Attribute/Flag Erasure Loop
]]
            ...
            if self:IsA("BasePart") and self.Name == "HumanoidRootPart" then
                if key == "AssemblyLinearVelocity" then
                    ...
                    if real.Magnitude > 60 then
                        return Vector3.new(
                            math.clamp(real.X, -50, 50),
                            real.Y,
                            math.clamp(real.Z, -50, 50)
                        )
                    end
                end
...
-- ─── STANDALONE KILL (for manual button) ────────────
...
do
    ...
    Notify("🛡️ AC: "..profile.name.." — 10-Layer Venv Ready",T.OK)
    ...
    print(string.format([[
    ═══════════════════════════════════════
     ⚡ XENO v%s — 10-Layer Venv AC Build
     AC Profile: %s
     Layers: L1-L10 (Venv Spoofing Active)
